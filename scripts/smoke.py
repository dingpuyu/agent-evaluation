...
def main() -> None:
    ...
    if not contract.get("flow") or len(dataset.get("cases") or []) < 4 or not cases:
        raise RuntimeError("target contract, production-shaped dataset or bad cases are not ready")
    ...
    if run.get("status") != "completed" or experiment.get("status") != "completed":
        raise RuntimeError("evaluation smoke did not complete")
    print(json.dumps({
        "status": "passed",
        "business_nodes": len(contract["flow"]),
        "dataset_cases": len(dataset["cases"]),
        "diagnosis": {"run_id": run["run_id"], "root_cause": run["report"]["root_cause"], "metrics": len(run["metrics"])},
        "prompt_experiment": {"experiment_id": experiment["experiment_id"], "baseline": experiment["baseline"], "candidate": experiment["candidate"], "regressions": experiment["regressed_cases"]},
