...
当前可信度状态（`document-quality/2`）：已修复标识符子串误判、零覆盖率伪满分、跨文档证据串用、单变量元数据漏检。无标注指标显示 `null / not_evaluated / n=0` 并阻断相关硬门禁；历史报告保留，但旧 PASS 不再授权晋级。现有 4 条 Holdout 缺少噪声清洗与来源定位标注，不能据此宣称完整链路通过。RAG 新增一次上传、一次解析、两套 Chunk 的成对导出，避免重新生成文件或重复 OCR 引入实验混杂。41 项确定性测试及真实本地 HTTP 探针已验证，不产生模型调用费用。详见 [评测可信度修复与验证记录](docs/document-quality-evaluator-integrity-report.md)。下文 Phase A–F 数值为旧规则下的历史记录，其通过/晋级结论须按新规则复核。
...
Phase F 没有重跑 Phase E 已曝光的盲测，而是构造 4 个全新 Case、8 份文档，验证短型号/长型号、基础型号/扩展型号、`4.2/14.2` 和前缀批次号。历史真实 Holdout 得到 `3/4 → 4/4`，Hit@5=`1.0`、MRR=`1.0`、Wrong Document=`0`、证据跨度 `0.75 → 1.0`；旧规则曾给出 `regression-ready`。新规则下该晋级结论已失效：原 Snapshot 仍视为已消费，不能通过更换 Candidate 或评测器重新盲测。完整历史证据见 [Phase F 独立 Holdout 报告](docs/document-quality-phase-f-independent-holdout-report.md)。

现在这条闭环已经进入网页工作台：在 RAG 项目使用 `make document-quality-export-pair` 一次导出 `400/100` 与 `700/80` 的无索引 Artifact 后，可直接上传到 `/document-quality`。平台检查原文件哈希、元数据、Document IR、清洗结果与非 Chunk 配置一致，只允许 Chunk Profile 变化。历史 Retrieval 对照显示：两组 Document Hit@5 和 MRR 都是 `1.0`，而单 Chunk 证据完整率从 `0 → 1.0`，说明文档命中不等于证据完整。`700/80` 仍是待验证的候选，不是生产最优参数；后续须采用成对导出和新版门禁，不能直接沿用旧成绩。操作和安全边界见 [Document Quality Lab](docs/document-quality-lab.md)。
