...
function renderResult(experiment) {
  ...
  $("#result").innerHTML = `<div class="decision-banner ${passed ? "" : "hold"}"><span>${passed ? "PASS" : "HOLD"}</span><div><b>${passed ? "Development Retrieval 门禁通过，可申请盲测" : "候选策略未通过，保持 Baseline"}</b><p>${h(experiment.comparison.recommendation)}</p></div><code>${h(experiment.experiment_id)}</code></div>
    ...
    ${sandboxTrace}<div class="metric-table"><div class="metric-row head"><span>METRIC</span><span>BASELINE</span><span>CANDIDATE</span><span>DELTA</span><span>DIRECTION</span></div>${metrics}</div>
