...
- 已接入临时 Milvus Collection 的真实 Retrieval 层，并补齐 PDF Page、XLSX Sheet/Cell Range/Heading Path 的结构化引用硬门禁；正确文档但错误定位会 HOLD。
- 下一步冻结同一 Candidate，依次运行一次性 Holdout 与 Regression；任何关键字段、引用、安全或新增回退都阻止晋级。
